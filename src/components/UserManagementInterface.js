...
const UserManagementInterface = () => {
  ...
  const fetchUsers = async () => {
    try {
      ...
      setAllRoles([...new Set([...response.data.map(user => user.role)])])
  ...
  const handleDelete = (id, fromRow = false) => {
    if (!fromRow && !selectedRows.includes(id)) {
  ...
  return (
    <div className="container">
      ...
      {isModalOpen && (
        <EditModal handleSaveRow={handleSaveRow} allRoles={allRoles} editRowData={editRowData} setIsModalOpen={setIsModalOpen} />
      )}
    </div>
  );
};
